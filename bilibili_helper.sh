...
if [ -f "$(dirname "$0")/utils_env.sh" ]; then
	...
	wget -q -O utils_env.sh https://ghproxy.com/https://raw.githubusercontent.com/Whuihuan/checkinpanel/master/utils_env.sh
	source "$(dirname "$0")/utils_env.sh"
fi
get_some_path
...
latest=$(curl -s https://api.github.com/repos/Whuihuan/bili/releases/latest)
